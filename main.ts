(() => {
    ...
    /**
     * Helper function for factorial calculation (Gamma function).
     */
    function gamma(n: number): number {
        // Factorial function approximation using Lanczos formula for Gamma function
        // Necessary for non-integer factorials, but for a simple calculator,
        // we'll primarily use the loop for integers and fall back to Gamma for float safety.
    ...
    function tokenize(expr: string): string[] {
        ...
        const singleCharSymbols: string = "+-*/^()√"; 

        while (i < expr.length) {
            const char: string = expr[i];
            
            // 1. Check for functions first
            let funcFound: boolean = false;
    ...
    function toPostfix(tokens: string[]): string[] {
        ...
        const rightAssociative: Set<string> = new Set([
            "^", "log", "sqrt", "sin", "cos", "tan", "ln", "asin", "acos", "atan", "e^x",
            "abs", "fact", "1/x"
        ]);
        
        const isOperatorOrFunction = (token: string): boolean => 
    ...
    calculator.addEventListener('click', (e) => {
        ...
        const shiftButton = calculator.querySelector('[data-value="shift"]') as HTMLButtonElement;
